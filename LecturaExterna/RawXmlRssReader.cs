...
namespace Persistencia
{
    ...
    /// </summary>
    public class RawXmlRssReader : LectorRss
    {
        public override IEnumerable<ItemRss> Read(Uri pUrl)
        {
            if (pUrl == null)
            {
            ...
            foreach (XmlNode bRssXmlItem in mRssXmlDocument.SelectNodes("//channel/item"))
            {
                mRssItems.Add(new ItemRss
                {
                    ...
                    Url = new Uri(RawXmlRssReader.GetXmlNodeValue<String>(bRssXmlItem, "link")).ToString(),
                    FechaDePublicacion = RawXmlRssReader.GetXmlNodeValue<DateTime?>(bRssXmlItem, "pubDate")
                });
            }
